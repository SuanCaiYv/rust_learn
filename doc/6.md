...
从叶子结点开始，到执行器，产生了一个链式结构，每一个状态机内部包含多个状态机。对于父状态机，需要判断当前子状态机的返回结果，如果是`Yield`则保持此状态不变并返回同样的`Yield`，如果是`Returned`则取出此子状态机轮询结果，继续下一步，直达下一个子状态机，循环直到返回自己的`Returned`。

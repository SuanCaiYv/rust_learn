...
指出了存储类型的合法地址的要求。一般是2^0-2^n，类型必须存储在n倍于填充的内存地址。

#### 大小

大小是一块囊括了全部成员和其填充的连续内存的尺寸。它是此类型填充的n倍，注意有些类型size为0，0可以作为任何填充的任意倍数，比如：[u16; 0]的size是0，填充为2。
...
- 指针和引用：具有相同的大小和填充，而对于指向DST的指针和引用而言，大小则是两倍于usize。
- 数组：大小等于元素个数n * 元素大小，填充为元素的填充
- 切片：同上
- str：同[u8]
- 元组：
